...
SELECT UNIQUE region, PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY shipment_cost) OVER (PARTITION BY region) AS 'PrimeiroQuadrante',
PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY shipment_cost) OVER (PARTITION BY region) AS 'Mediana',
PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY shipment_cost) OVER (PARTITION BY region) AS 'TerceiroQuadrante',
(PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY shipment_cost) OVER (PARTITION BY region)) - 
(PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY shipment_cost) OVER (PARTITION BY region)) AS 'Intervalo Interquartil',
(PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY shipment_cost) OVER (PARTITION BY region)) + 
1.5 * (PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY shipment_cost) OVER (PARTITION BY region)) - 
(PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY shipment_cost) OVER (PARTITION BY region)) AS 'Limite dos Outliers'
FROM entregas;
